...
public class TableManagementPanelDAO extends JPanel {
    ...
    public TableManagementPanelDAO() {
        ...
        JButton viewAvailableButton = new JButton("View Available Tables");
        ...
        viewAvailableButton.addActionListener(e -> viewAvailableTables());
    ...
    private List<String[]> getAvailableTables(String dateTime) {
        List<String[]> availableTables = new ArrayList<>();
        String query = "SELECT * FROM Tables WHERE table_id NOT IN (SELECT table_id FROM Reservations WHERE reservation_time <= ? AND ADDTIME(reservation_time, SEC_TO_TIME(duration * 60)) > ?)";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, dateTime);
            stmt.setString(2, dateTime);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                availableTables.add(new String[]{
                    String.valueOf(rs.getInt("table_id")),
                    rs.getString("table_number"),
                    String.valueOf(rs.getInt("capacity")),
                    rs.getString("location"),
    ...
    private void viewAvailableTables() {
        ...
        if (dateTime != null) {
            List<String[]> availableTables = getAvailableTables(dateTime);
            tableModel.setRowCount(0); // Clear existing data
            for (String[] row : availableTables) {
                tableModel.addRow(row);
            }
        }
    }
